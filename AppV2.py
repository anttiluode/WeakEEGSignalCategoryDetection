"""
EEG Category Brain Explorer - With Contrast Analysis
"""
...
class CategoryBrainExplorer(tk.Tk):
    ...
    def setup_left_panel(self, parent):
        ...
        tk.Button(source_frame, text="Contrast Analysis (High vs Low)",
                 command=self.localize_contrast, bg="#FF5722", fg="white",
                 font=("Arial", 9, "bold")).grid(row=3, column=0, columnspan=2, pady=5)
    ...
    def plot_probability_distribution(self, cat_name):
        ...
        self.ax_dist.axvline(p25, color='blue', linestyle=':', linewidth=1.5, label='25th %ile')
        self.ax_dist.axvline(p75, color='orange', linestyle=':', linewidth=1.5, label='75th %ile')
    ...
    def localize_contrast(self):
        ...
        thread = threading.Thread(target=self._run_contrast_localization, args=(cat_name,), daemon=True)
        thread.start()
    ...
    def _run_contrast_localization(self, cat_name):
        try:
            self.log(f"\n{'='*60}")
            self.log(f"CONTRAST LOCALIZATION: {cat_name.upper()}")
            self.log("High vs Low Probability Trials")
            self.log(f"{'='*60}")
            
            info = self.category_trials[cat_name]
            
            # Split by probability quartiles
            high_thresh = np.percentile(info['probabilities'], 75)
            low_thresh = np.percentile(info['probabilities'], 25)
            
            high_mask = info['probabilities'] >= high_thresh
            low_mask = info['probabilities'] <= low_thresh
            
            high_eeg = info['eeg_data'][high_mask]
            low_eeg = info['eeg_data'][low_mask]
            
            self.log(f"High-prob trials (>75th percentile): {len(high_eeg)}, threshold={high_thresh:.3f}")
            self.log(f"Low-prob trials (<25th percentile): {len(low_eeg)}, threshold={low_thresh:.3f}")
            
            if len(high_eeg) < 3 or len(low_eeg) < 3:
                messagebox.showwarning("Warning", "Need at least 3 trials in each group")
                return
            
            self.progress['value'] = 20
            
            # Compute difference
            avg_high = high_eeg.mean(axis=0)
            avg_low = low_eeg.mean(axis=0)
            diff_eeg = avg_high - avg_low
            
            self.log("Computing EEG difference (high - low)...")
            self.log(f"Difference signal range: {diff_eeg.min():.3e} to {diff_eeg.max():.3e}")
            
            # Create MNE Raw object from difference
            raw_diff = create_mne_raw_from_alljoined(diff_eeg)
            
            # Preprocess (lighter filtering for difference signal)
            raw_diff.filter(0.5, 50, fir_design='firwin', verbose=False)
            raw_diff.set_eeg_reference('average', projection=True, verbose=False)
            
            t_start = self.time_start.get()
            t_end = min(self.time_end.get(), raw_diff.times[-1])
            raw_diff.crop(tmin=t_start, tmax=t_end)
            ...
            fwd = mne.make_forward_solution(raw_diff.info, trans='fsaverage',
                                           src=src, bem=bem_sol,
                                           eeg=True, meg=False, verbose=False)
            
            noise_cov = mne.compute_raw_covariance(raw_diff, verbose=False)
            inv = mne.minimum_norm.make_inverse_operator(raw_diff.info, fwd, noise_cov,
                                                         loose=0.2, depth=0.8, verbose=False)
            
            self.progress['value'] = 80
            
            stc_diff = mne.minimum_norm.apply_inverse_raw(raw_diff, inv, lambda2=1/9,
                                                          method='sLORETA', verbose=False)
            
            self.log("Contrast source reconstruction complete")
            self.progress['value'] = 90
            
            self.after(0, lambda: self._visualize_contrast(stc_diff, cat_name, subjects_dir))
    ...
    def _visualize_contrast(self, stc_diff, cat_name, subjects_dir):
        try:
            # For contrast, we use absolute value to show differences in either direction
            abs_data = np.abs(stc_diff.data)
            stc_contrast = mne.SourceEstimate(
                abs_data, vertices=stc_diff.vertices,
                tmin=stc_diff.tmin, tstep=stc_diff.tstep, subject='fsaverage'
            )
            ...
            brain = stc_contrast.plot(
                ...
                time_label=f"{cat_name} - CONTRAST (High-Low) (t=%0.2fs)",
                size=(1200, 800),
                background='white',
                verbose=False
            )
            ...
            self.brain_figures.append(brain)
            self.log(f"3D contrast visualization created for {cat_name}")
            self.log("This shows regions differentially active in high vs low detection trials")
